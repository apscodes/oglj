...
public class Mat4f
{
	 ...
	 * 
	 * @param	Vec3f	The vector holding the x-basis to set
	...
	public void setXBasis(Vec3f v)
	{
		...
		if ( !v.lengthIsApproximately(1.0f) )
		{
			v.normalise();
		}
	 ...
	 * {@link #setFromArray(float[])} and {@link #Mat4f(float[])} methods. 
	 * 
	 * @param	Vec3f	The vector holding the y-basis to set
	 * @see		#setFromArray(float[])
	 * @see		#Mat4f(float[])
	 */
	 ...
	 * @return	This Mat4f for chaining.
	 */
	public Mat4f translate(float xAmount, float yAmount, float zAmount)
	{
		return translate( new Vec3f(xAmount, yAmount, zAmount) );
	}
	...
	// This rotates a matrix about an axis, and it FUCKING WORKS
	public Mat4f rotateRads2(float angleRads, Vec3f axis)
	{
		
		Mat4f dest = new Mat4f(1.0f);
	 ...
	 * @return				This rotated matrix
	 */
	public Mat4f rotateAboutLocalAxisDegs(float rotDegs, Vec3f axis)
	{
		return rotateAboutLocalAxisRads(rotDegs * DEGS_TO_RADS, axis);
	}
	
	/**
	 * Rotate this matrix about a world-space axis by an angle specified in radians.
	 * <p>
	 * By a 'world-space' axis we mean an axis specified with regard to standard
	 * OpenGL world space, where the positive X-axis runs to the right, the positive
	 * Y-axis runs vertically upwards and the positive Z-axis runs outwards from the
	 * screen.
	 * 
	 * @param	angleRads	The angle to rotate the matrix in radians.
	...
	public Mat4f rotateAboutWorldAxisRads(float angleRads, Vec3f worldAxis)
	{
		...
		Vec3f rotatedXAxis = Vec3f.rotateVectorAboutAxisRads(xAxis, angleRads, worldAxis);
		Vec3f rotatedYAxis = Vec3f.rotateVectorAboutAxisRads(yAxis, angleRads, worldAxis);
		Vec3f rotatedZAxis = Vec3f.rotateVectorAboutAxisRads(zAxis, angleRads, worldAxis);
	...
	/**
	 * Rotate this matrix about a world-space axis by an angle specified in degrees.
	 * <p>
	 * By a 'world-space' axis we mean an axis specified with regard to standard
	 * OpenGL world space, where the positive X-axis runs to the right, the positive
	 * Y-axis runs vertically upwards and the positive Z-axis runs outwards from the
	 * screen.
	 * <p>
	 * Internally this method simply converts degrees to radians and calls the
	 * {@link #rotateAboutWorldAxisRads(float, Vec3f)} method.
	 * 
	 * @param	angleDegs	The angle to rotate the matrix in degrees.
	 * @param	worldAxis	The world-space axis around which to rotate the matrix.
	 * @return				This rotated matrix
	 */
	public Mat4f rotateAboutWorldAxisDegs(float angleDegs, Vec3f worldAxis)
	{
		return rotateAboutWorldAxisRads(angleDegs * DEGS_TO_RADS, worldAxis);
	}

	
	//FIXME: WARNING WARNING WARNING!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	// This either does NOT work properly, or it is not the right tool for the job to rotate
	// the camera matrix! If you use this to rotate the camera matrix the entire fucking thing
	// turns on its side when you look left and right
	/**
	 ...
	 * @return				The rotated matrix.
	 */	
	public static Mat4f rotateMatrixAboutWorldAxisRads(Mat4f matrix, float angleRads, Vec3f worldAxis)
	{
	 ...
	 * That is, if the positive X-axis (that is, the axis {@code Vec3f(1.0f, 0.0f, 0.0f)}) is
	 * specified as the axis of rotation, then the positive X-axis of the provided rotation matrix
	 * is used as the rotation matrix, which is not necessarily (or in fact likely to be) the same
	 * direction as the global 'world-space' X-axis.
	 * 
	 * @param	matrix		The matrix to rotate.
	 * @param	angleRads	The angle to rotate the matrix about in radians.
	 * @param	axis		The relative axis to rotate about.
	 * @return				A rotated matrix.
	 */
	public static Mat4f rotateMatrixAboutLocalAxisRads(Mat4f matrix, float angleRads, Vec3f localAxis)
	{
		System.out.println("Rotating about local axis   : " + localAxis);
		
		// Transform the local rotation axis into world space
		Vec3f worldSpaceAxis = matrix.transformDirection(localAxis);
		
		System.out.println("Converted into world-space axis: " + worldSpaceAxis);
		
		// With that done, we can now use the world-space axis method to perform the matrix rotation
		return Mat4f.rotateMatrixAboutWorldAxisRads(matrix, angleRads, worldSpaceAxis);
	}
	...
	{
		// Sanity check the aspect ratio and throw an illegal argument exception if it is invalid
		if (aspectRatio < 0.0f)
		{	
			throw new IllegalArgumentException("Aspect ratio cannot be negative.");
		}
		
		// Ensure zNear and zFar are positive
		if (zNear <= 0.0f || zFar <= 0.0f)
		{
			throw new IllegalArgumentException("The values of zNear and zFar must be positive.");
		}
		
		// Calculate the frustum length and throw an illegal argument exception if it is invalid
		...
		// Sanity check the vertical field of view and throw an illegal argument exception if it is invalid
